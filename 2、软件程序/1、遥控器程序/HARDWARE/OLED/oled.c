...
#include "oledfont.h"  	 
#include "delay.h"
#include "oled_BMP.h"		//BMP图像库
#include "oled_CN.h"		//中文字库
...
{      			    
    ...
    for(t=0;t<12;t++)
		{
				OLED_WR_Byte(Hzk_SMALL[2*no][t],OLED_DATA);
				adder+=1;
     }	
		OLED_Set_Pos(x,y+1);	
    for(t=0;t<12;t++)
			{	
				OLED_WR_Byte(Hzk_SMALL[2*no+1][t],OLED_DATA);
				adder+=1;
      }					
}
