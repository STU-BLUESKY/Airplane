...
#define CONSTANTS_ONE_G					9.78634f		/* m/s^2		*/
...
struct _trans{
     struct _int_16 origin;   //原始值		最原始数据
	   struct _int_16 averag;   //平均值
	   struct _int_16 histor;   //历史值
	   struct _int_16 quiet;    //静态值
		 struct _float cal;		    //最原始数据除以灵敏度
...
---------------------------------------------------*/
#define KALMAN_Q        2.0		//0.03			2.0 -- 20180130
#define KALMAN_R        8.000		//8.000
/*----------------------------------------------
		MPU6050量程和灵敏度定义
-----------------------------------------------*/
#define ORIGINAL_OUTPUT			 (0)								//量程定义
#define ACC_FULLSCALE        (2)
#define GYRO_FULLSCALE			 (1000)							//	250/500/2000 度/每秒(dps)
/*-----------------------------------------------*/
#if ORIGINAL_OUTPUT == 0												//加速度灵敏度
	#if  ACC_FULLSCALE  == 2
